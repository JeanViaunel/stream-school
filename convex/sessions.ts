...
// --- Class session lobby (Stream join only after teacher admits in Convex) ---

async function requireUserForLobby(ctx: QueryCtx | MutationCtx) {
  const identity = await ctx.auth.getUserIdentity();
  if (!identity) throw new Error("Not authenticated");
  const user = await ctx.db
    .query("users")
...
async function canAccessClassForLobby(
...
export const requestLobbyAccess = mutation({
  ...
  handler: async (ctx, args) => {
    const user = await requireUserForLobby(ctx);
    const session = await ctx.db.get(args.sessionId);
    if (!session) throw new Error("Session not found");
    if (session.endedAt !== undefined) throw new Error("Session has ended");

    const allowed = await canAccessClassForLobby(ctx, session.classId, user);
