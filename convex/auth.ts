...
export const registerStudent = action({
  ...
  handler: async (
    ...
    if (isUnder13) {
      ...
      if (!args.parentEmail) {
        throw new Error("Parent email is required for users under 13 years old (COPPA compliance)");
      }
    ...
    if (parentalConsentStatus !== "pending") {
      ...
      convexAuthToken = await ctx.runAction(internal.convexJwt.signConvexAuthToken, {
        username: args.username,
      });
    }
...
export const verifyParentalConsent = action({
  ...
  handler: async (
    ...
    return {
      success: true,
      message: "Parental consent verified successfully. The student can now log in.",
      userId: user._id,
    };
  },
});
...
export const sendConsentEmail = internalAction({
  ...
  handler: async (
    ...
    console.log(`[COPPA] Student: ${args.studentDisplayName} (${args.studentUsername})`);
  ...
  handler: async (
    ...
    if (user.parentalConsentStatus === "pending") {
      return {
        canLogin: false,
        parentalConsentStatus: "pending",
        message: "Parental consent is pending. Please ask your parent to check their email and verify consent.",
      };
    }
